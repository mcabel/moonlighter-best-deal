...
namespace QuickPrice
{
	...
	public class Plugin : BaseUnityPlugin
	{
		...
		{
			LOG = base.Logger;
			Debug = base.Config.Bind("0. Debug", "Debug", defaultValue: false, new ConfigDescription("Enables debug logging.", null, new ConfigurationManagerAttributes
			{
				Order = 99
			}));
			EnableNotebookHighlight = base.Config.Bind("1. Notebook", "Enable Notebook Highlight", defaultValue: true, new ConfigDescription("Highlights optimal prices in the notebook.", null, new ConfigurationManagerAttributes
			{
				Order = 50
			}));
			EnableAutoPrice = base.Config.Bind("2. Auto Price", "Enable Auto Price", defaultValue: false, new ConfigDescription("Automatically sets the optimal price when placing items on the showcase.", null, new ConfigurationManagerAttributes
			{
				Order = 49
			}));
			PriceTier = base.Config.Bind("2. Auto Price", "Price Tier", "MarketPrice", new ConfigDescription("Target price tier for auto pricing.", new AcceptableValueList<string>("MaxPriceInTooCheap", "MaxCorrectPrice", "MaxPriceInExpensive", "MaxPriceInTooExpensive", "MarketPrice"), new ConfigurationManagerAttributes
			{
				Order = 48
			}));
		...
		private TierHotkey MakeTier(int order, string label, string tier, string gpDefault, KeyboardShortcut kbDefault)
		{
			...
			tierHotkey.Gamepad = base.Config.Bind("3. Hotkeys", label + " Gamepad", gpDefault, new ConfigDescription("Gamepad button for " + label + ".", new AcceptableValueList<string>(GamepadOptions), new ConfigurationManagerAttributes
			{
				Order = order
			}));
			tierHotkey.Keyboard = base.Config.Bind("3. Hotkeys", label + " Keyboard", kbDefault, new ConfigDescription("Keyboard binding for " + label + ".", null, new ConfigurationManagerAttributes
			{
				Order = order - 1
			}));
		...
		public void Update()
		{
			...
			if ((Object)(object)currentSelectedGameObject == (Object)null || (Object)(object)currentSelectedGameObject.GetComponent<ButtonPrizeHandler>() == (Object)null)
			{
				return;
			}
			ShowcaseSlotGUI componentInParent = currentSelectedGameObject.GetComponentInParent<ShowcaseSlotGUI>();
			if ((Object)(object)componentInParent == (Object)null || (Object)(object)componentInParent.itemStack == (Object)null)
			{
				return;
			}
			for (int i = 0; i < _hotkeys.Length; i++)
			{
				...
				if (value != "None" && GamepadMap.TryGetValue(value, out var value2) && Input.GetKeyDown(value2))
				{
					ApplyHotkeyPrice(componentInParent, tierHotkey.Tier);
					break;
				}
				if (value != "None" && AnalogDirMap.TryGetValue(value, out (int, float) value3) && IsAnalogDirActive(value, value3.Item1, value3.Item2))
				{
					ApplyHotkeyPrice(componentInParent, tierHotkey.Tier);
					break;
				}
		...
		private bool IsAnalogDirActive(string name, int axisIdx, float dir)
		{
			...
			bool flag = ((dir > 0f) ? (analogAxis > 0.7f) : (analogAxis < -0.7f));
			bool flag2 = _analogWasActive.TryGetValue(name, out var value) & value;
			_analogWasActive[name] = flag;
			if (flag)
			{
				return !flag2;
			}
		...
		private static void ApplyHotkeyPrice(ShowcaseSlotGUI slotGUI, string tier)
		{
			...
			case "MaxPriceInTooCheap":
				num = instance.GetMinCorrectPrice(master, isCool) - 1;
				break;
			case "MaxCorrectPrice":
				num = instance.GetMaxCorrectPrice(master, hasHighBudget: false, isCool);
				break;
			case "MaxPriceInExpensive":
				num = instance.GetMaxOverpricedPrice(master, hasHighBudget: false, isCool);
				break;
			case "MaxPriceInTooExpensive":
				num = instance.GetTooExpensiveLimitPrice(master, hasHighBudget: false, isCool);
				break;
			case "MarketPrice":
				num = instance.GetCorrectPriceWithPopularity(master, isCool);
				break;
			default:
				num = 0;
				break;
			...
			if (num2 > 0)
			{
				itemStack.unitSellingPrice = num2;
				AccessTools.Method(typeof(ShowcaseSlotGUI), "SetLabelNumber")?.Invoke(slotGUI, new object[2] { num2, itemStack.Quantity });
				slotGUI.ownerShowcase.itemPositions[slotGUI.slotIndex].SetPrice(num2 * itemStack.Quantity);
				instance.SetLastSettedPrice(master, num2);
				if (Debug.Value)
				{
					LOG.LogInfo($"Hotkey priced '{master.nameKey}' to {num2} (tier: {tier}, isCool: {isCool})");
				}
			}
		}
	}
